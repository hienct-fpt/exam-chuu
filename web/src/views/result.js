...
export async function renderResult({ app }, attemptId) {
  ...
  const unsub = watchAttempt(attemptId, async (a) => {
    ...
    if (a.status !== 'graded') {
      app.innerHTML = `<div class="card"><h1>${esc(title)}</h1>
    ...
    app.innerHTML = `
      <div class="card">
        ...
        <p><a href="#/exam/${a.examId}">もう一度挑戦</a> · <a href="#/">一覧へ</a>${a.emailHtml ? ' · <a href="#" id="showmail">メールプレビュー (mock)</a>' : ''}</p>
      </div>
      <div class="card"><h2>大問別</h2><table><tr><th>大問</th><th>正解</th><th>正答率</th></tr>
        ${Object.values(r.perBig).sort((x, y) => x.big - y.big).map((b) => `<tr><td>${b.big}</td><td>${b.correct} / ${b.items}</td><td>${Math.round(100 * b.correct / b.items)}%</td></tr>`).join('')}
      </table></div>
      <div class="card"><h2>全問</h2>
        <label class="muted"><input type="checkbox" id="onlywrong"> 間違いのみ表示</label>
        <table id="items"><tr><th></th><th>問</th><th>あなたの回答</th><th>正答</th><th>問題</th></tr>
