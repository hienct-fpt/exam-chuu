...
export async function renderHistory({ app }) {
  ...
  app.innerHTML = `<div class="card"><h1>履歴</h1>
    ${attempts.length ? `<table><tr><th>日時</th><th>試験</th><th>範囲</th><th>状態</th><th>得点</th><th></th></tr>
    ${attempts.map((a) => `<tr>
      ...
      <td>${a.status === 'in_progress' ? `<a href="#/exam/${a.examId}${a.gradeFilter ? `?g=${a.gradeFilter}` : ''}">続ける</a>` : `<a href="#/result/${a.id}">結果</a>`}</td>
